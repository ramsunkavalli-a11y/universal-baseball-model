#!/usr/bin/env python3
"""Freeze Marcel and no-aging 2026 hitter forecasts without reading 2026 outcomes."""
...
from audit_joint_hitter_aging_2025 import _counterfactual_no_aging
...
OUTPUT_ROOT = Path("model_artifacts/hitter-aging-2026-confirmation-forecast-2026-09-10")
CONTRACT = Path("docs/hitter-aging-2026-confirmation-contract.md")


def main() -> int:
    projection_path = PROJECTION_ROOT / "tables/hitter-expected-war-paths.parquet"
    source_path = SKILL_ROOT / "tables/mlb_hitting_components.parquet"
    ...
    if projections.select("player_id").to_series().to_list() != no_aging.select(
        "player_id"
    ).to_series().to_list():
        raise ValueError("aging confirmation candidates differ in player denominator")
    unchanged = [
        "expected_mlb_pa", "mlb_active_probability", "conditional_mlb_pa",
        "positional_runs_per_600", "replacement_runs_per_600",
        "baserunning_runs_per_600", "defense_runs_per_600",
    ]
    ...
    storage = {
        "marcel": write_canonical_parquet(
            projections, OUTPUT_ROOT / "marcel-aging.parquet",
            table_name="hitter_aging_2026_marcel_forecast",
        ).as_record(),
        "no_aging": write_canonical_parquet(
            no_aging, OUTPUT_ROOT / "no-aging.parquet",
            table_name="hitter_aging_2026_no_aging_forecast",
        ).as_record(),
    }
    ...
    report = {
        ...
        "marcel_expected_war": float(projections.get_column("expected_war").sum()),
