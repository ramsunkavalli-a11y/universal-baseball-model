#!/usr/bin/env python3
"""Execute the frozen one-shot disclosed comparison for S0."""
...
BASELINES = ("C0_NESTED_EB", "B1_MARCEL_345_K1200")
CANDIDATE = "S0_C0_MARCEL_STABILITY_BLEND"
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--contract",
        type=Path,
        default=Path("docs/hitter-v2-S0-scoring-contract.json"),
    )
    parser.add_argument(
        "--s0-root",
        type=Path,
        default=Path("reports/generated/hitter-v2-S0-fit/tables"),
    )
    parser.add_argument(
        "--baseline-root",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/generated/hitter-v2-S0-comparison/report.json"),
    )
    return parser.parse_args()


def _verify(contract: dict[str, object], runner: Path) -> None:
    if contract["status"] != "frozen_before_first_disclosed_S0_score":
        raise ValueError("S0 scoring contract is not frozen")
    if sha256_file(runner) != contract["runner_sha256"]:
        raise ValueError("S0 scoring runner changed after freeze")
    if contract["one_shot_scoring_authorized"] is not True:
        raise ValueError("S0 one-shot scoring is not authorized")
    for boundary in (
        ...
        if contract[boundary] is not False:
            raise ValueError(f"S0 scoring improperly opened {boundary}")
    for fold in FOLDS:
        for record in contract["folds"][fold]["inputs"]:
            path = Path(record["path"])
            if sha256_file(path) != record["sha256"]:
                raise ValueError(f"S0 scoring input changed: {path}")
...
def _correlation_gate(metrics: dict[str, dict[str, object]]) -> dict[str, object]:
    ...
    for metric in CORRELATION_METRICS:
        baseline_id = max(
            BASELINES, key=lambda model: (float(metrics[model][metric]), model)
        )
            ...
            if supported:
                for view in ("player", "pa"):
                    gates[view] = subgroup_reversal_gate(
                        ...
                        {
                            baseline: scores[baseline]["metrics"][view]
...
def _pooled_gate(metrics: dict[str, dict[str, object]]) -> dict[str, object]:
    ...
    for view in ("player", "pa"):
        ...
        for metric in PRIMARY_METRICS:
            ...
            comparisons[view][metric] = {
                ...
                "minimum": POOLED_RELATIVE_MINIMUM[metric],
    ...
    return {
        ...
        "pass": all(
            row["pass"]
            for view in comparisons.values()
...
def main() -> int:
    ...
    for fold in FOLDS:
        slug = fold.lower()
        predictions = {
            "C0_NESTED_EB": pl.read_parquet(
                args.baseline_root / slug / "c0_nested_eb_predictions.parquet"
            ),
            "B1_MARCEL_345_K1200": pl.read_parquet(
                args.baseline_root / slug / "b1_marcel_345_k1200_predictions.parquet"
            ),
            CANDIDATE: pl.read_parquet(args.s0_root / slug / "s0_predictions.parquet"),
        }
        ...
        surfaces = {
            model: build_player_scoring_surface(
                frame, target, model_id=model, fold_id=fold
            )
            for model, frame in predictions.items()
        }
        ...
        primary = {
            view: primary_gate(
                ...
                {baseline: metrics[baseline][view] for baseline in BASELINES},
            )
            for view in ("player", "pa")
        }
        correlations = {
    ...
    pooled_gate = _pooled_gate(pooled_metrics)
    promotion_pass = (
        ...
        and pooled_gate["pass"]
    )
    report = {
        "schema_version": "0.1",
        "program": "hitter_v2",
        "stage": "S0_one_shot_disclosed_comparison",
        "status": "S0_passed" if promotion_pass else "S0_failed_final",
        "folds": fold_reports,
        ...
        "next_gate": (
            "review_then_separately_authorize_L0_target_free_fit"
            if promotion_pass
        ...
        json.dumps(
            {
                ...
                "pooled_gate_pass": pooled_gate["pass"],
            },
            indent=2,
            sort_keys=True,
        )
