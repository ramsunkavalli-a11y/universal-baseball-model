...
SPECS = (
    {"season": 2025, "kind": "batting", "tag": "season_player_batting", "asset": "2025_aaa_season_batting_stats.csv", "sport_label": "Triple-A"},
    {"season": 2025, "kind": "batting", "tag": "season_player_batting", "asset": "2025_rk_season_batting_stats.csv", "sport_label": "Rookie"},
    {"season": 2025, "kind": "pitching", "tag": "season_player_pitching", "asset": "2025_aaa_season_pitching_stats.csv", "sport_label": "Triple-A"},
    {"season": 2025, "kind": "pitching", "tag": "season_player_pitching", "asset": "2025_rk_season_pitching_stats.csv", "sport_label": "Rookie"},
)
...
def _grain_profile(frame: pl.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "duplicate_extra_row_count": int(duplicates.select((pl.col("len") - 1).sum()).item() or 0),
...
def _accounting_profile(frame: pl.DataFrame, kind: str) -> dict[str, Any]:
    if kind == "batting":
        ...
        components = ["batting_at_bats", "batting_base_on_balls", "batting_hit_by_pitch", "batting_sac_bunts", "batting_sac_flies", "batting_catchers_interference_reached"]
        label = "PA = AB + BB + HBP + SH + SF + CI"
    else:
        total = "pitching_batters_faced"
        components = ["pitching_at_bats", "pitching_base_on_balls", "pitching_hit_batsmen", "pitching_sac_bunts", "pitching_sac_flies", "pitching_catchers_interference"]
        label = "BF = AB + BB + HBP + SH + SF + CI"

    missing = sorted({total, *components} - set(frame.columns))
    if missing:
        return {"available": False, "identity": label, "missing_columns": missing, "reason": "source does not expose every component required for exact accounting"}

    working = frame.with_columns(
        _source_numeric(total).alias("__total"),
        sum((_source_numeric(column) for column in components), start=pl.lit(0, dtype=pl.Int64)).alias("__components"),
    )
    mismatches = working.filter(pl.col("__total") != pl.col("__components"))
    difference_counts = Counter(int(value) for value in mismatches.select((pl.col("__total") - pl.col("__components")).alias("difference")).get_column("difference").to_list())
    return {
        "available": True,
        "identity": label,
        ...
        "mismatch_examples": mismatches.select([column for column in ["player_id", "team_id", "league_id", total, *components] if column in mismatches.columns]).head(15).to_dicts(),
    }
...
def _stats_endpoint(person_id: int, kind: str, sport_id: int, season: int) -> str:
    params = {"stats": "season", "group": "hitting" if kind == "batting" else "pitching", "season": season, "sportId": sport_id}
...
def _aggregate_source_player_league(frame: pl.DataFrame, *, person_id: int, league_id: int, fields: list[str]) -> tuple[dict[str, int], int]:
    subset = frame.filter((pl.col("player_id").cast(pl.Int64, strict=False) == person_id) & (pl.col("league_id").cast(pl.Int64, strict=False) == league_id))
    available = [field for field in fields if field in subset.columns]
    if subset.is_empty() or not available:
        return {}, subset.height
    values = subset.select([_source_numeric(field).sum().alias(field) for field in available]).row(0, named=True)
    return {field: int(value or 0) for field, value in values.items()}, subset.height


def _compare_player(*, frame: pl.DataFrame, kind: str, sample: Mapping[str, int], sport_id: int, season: int, session: Any) -> dict[str, Any]:
    ...
    source, source_row_count = _aggregate_source_player_league(frame, person_id=person_id, league_id=league_id, fields=list(compare_fields.values()))
    ...
    if len(official) == 1:
        ...
        for official_field, source_field in compare_fields.items():
            ...
            if source_value != official_value:
                differences[source_field] = {"source": source_value, "official": official_value, "official_field": official_field}
    ...
    return {
        ...
        "official_split_metadata": [{key: value for key, value in row.items() if key != "stat"} for row in official],
    }
...
def main() -> int:
    ...
    for spec in SPECS:
        ...
        metadata = download_file(f"{BASE_URL}/{spec['tag']}/{spec['asset']}", path, timeout_seconds=180)
        raw_frame = read_quarantined_csv(path)
        frame, normalization = standardize_armstjc_season_stats(raw_frame, spec["kind"])
    ...
    try:
        ...
        for spec, frame, metadata, normalization in loaded:
            selected = select_reconciliation_players(frame, spec["kind"], per_league=SAMPLE_PER_ACTUAL_LEAGUE)
            ...
            if selected and sport_id is None:
                raise RuntimeError(f"official sports endpoint lacks expected {spec['sport_label']!r}")
            for sample in selected:
                player_reports.append(_compare_player(frame=frame, kind=spec["kind"], sample=sample, sport_id=int(sport_id), season=int(spec["season"]), session=session))
    finally:
        session.close()

    available_accounting = [report["accounting"] for report in source_reports if report["accounting"].get("available")]
    payload = {
        ...
        "source_grain_all_unique": all(report["grain"]["duplicate_group_count"] == 0 for report in source_reports),
        "available_accounting_check_count": len(available_accounting),
        "source_available_accounting_all_exact": bool(available_accounting) and all(report.get("mismatch_row_count") == 0 for report in available_accounting),
        "official_sample_count": len(player_reports),
        "official_player_samples_all_exact": bool(player_reports) and all(report["exact"] for report in player_reports),
        "interpretation": "Completed 2025 source rows are compared to current official completed-season totals. Samples are chosen deterministically from each actual source league and exclude players who appeared in multiple source leagues, so the broader official sport total is comparable. Current official data is a certification oracle, not a historical-vintage rewrite.",
    }
    (report_dir / "season_stat_official_reconciliation.json").write_text(json.dumps(payload, indent=2, sort_keys=True, default=str), encoding="utf-8")

    lines = [
        "# Season-player aggregate certification", "", "Completed season under test: **2025**",
        ...
        f"- Deterministic official samples: {payload['official_sample_count']}; all exact={payload['official_player_samples_all_exact']}", "", "## Source files", "",
    ]
    for report in source_reports:
        accounting = report["accounting"]
        if accounting.get("available"):
            accounting_text = f"accounting mismatches {accounting.get('mismatch_row_count')}/{accounting.get('compared_row_count')}"
        else:
            accounting_text = f"accounting unavailable; missing {accounting.get('missing_columns', [])}"
        samples = ", ".join(f"league {sample['league_id']}→{sample['player_id']} ({sample['sample_volume']})" for sample in report["selected_reconciliation_players"])
        lines.append(f"- `{report['asset']}`: rows {report['grain']['row_count']:,}; duplicate grain groups {report['grain']['duplicate_group_count']}; {accounting_text}; samples [{samples}]")

    lines.extend(["", "## Official completed-season checks", ""])
    for report in player_reports:
        lines.append(f"- {report['kind']} player `{report['person_id']}`, source league `{report['league_id']}`: source team rows {report['source_team_row_count']}; official splits {report['official_split_count']}; compared fields {report['compared_field_count']}; differences {report['difference_count']}; exact={report['exact']}")
        if not report["exact"]:
            lines.append(f"  - diagnostic: differences `{report['differences']}`; official metadata `{report['official_split_metadata']}`")

    lines.extend(["", "A clean result would support using season-player aggregates as the historical outcome-count backbone, with PBP retained for contact profile and sampled official PBP retained for league run-value calibration.", ""])
    summary = "\n".join(lines)
