...
No season-end totals are used here.  Every count is derived from game-grain
Savant evidence that occurred before a future validation cutoff.
"""
...
MLB_LEVEL_GROUP = "MLB"
MLB_CAPABILITY_TIER = "mlb_savant_result_contact_profile_v1"
MLB_PARTICIPANT_AUTHORITY = "savant_official"

# True PA terminal events that are neither the two outcome core families nor a
# batted-ball/contact family are explicit known non-core outcomes.  Contact
# non-core/unknown status is handled independently by the certified contact
# classifier rather than inferred from terminal labels.
...
def build_mlb_current_talent_player_game_evidence(
    ...
    ``savant`` must already have certified AL/NL ``league_id`` assignment from
    :func:`assign_savant_actual_league`.  The function preserves one row per
    player × actual league × game in the summary and long-form core-bin counts in
    the profile.
    """
    ...
    observed_leagues = {
        int(value)
        for value in savant.get_column("league_id").cast(pl.Int64, strict=False).drop_nulls().unique().to_list()
    }
    ...
    outcome = terminal_games.group_by(
        ["season", "_game_date", "game_pk", "league_id", "player_id"]
    ).agg(
        pl.len().alias("batting_plate_appearances"),
        pl.col("events").is_in(sorted(BB_HBP_EVENT_TYPES)).sum().cast(pl.Int64).alias("bb_hbp_count"),
        pl.col("events").is_in(sorted(STRIKEOUT_EVENT_TYPES)).sum().cast(pl.Int64).alias("strikeout_count"),
        pl.col("events").is_in(sorted(KNOWN_SPECIAL_NONCONTACT_EVENT_TYPES)).sum().cast(pl.Int64).alias(
            "special_noncontact_count"
        ),
    )

    contacts = classify_mlb_savant_contacts(savant)
    contact_games = contacts.group_by(
        ...
        (pl.col("contact_profile_status") == "special_bunt").sum().cast(pl.Int64).alias("bunt_contact_count"),
        (pl.col("contact_profile_status") == "foul_air_excluded").sum().cast(pl.Int64).alias(
            "foul_air_excluded_count"
        ),
        pl.col("contact_profile_status").str.starts_with("unknown").sum().cast(pl.Int64).alias(
    ...
    joined = outcome.join(
        ...
        *[
        ...
        ).alias("known_non_core_event_count"),
    ).with_columns(
        (
    ...
    if joined.filter(pl.col("unknown_event_count") < 0).height:
        raise ValueError("MLB Current Talent evidence over-accounts player-game plate appearances")
